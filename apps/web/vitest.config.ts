...
//
// app/composables is in scope for the same reason and on the same terms: only
// the DOM-free modules there (dragGeometry) are covered, so `node` stays the
// right environment and no jsdom is needed.
export default defineConfig({
  test: {
    include: ["server/utils/__tests__/**/*.test.ts", "app/composables/__tests__/**/*.test.ts"],
    environment: "node",
  },
});
