...
- [x] `Plan` screen (`app/pages/plan.vue`) — no longer an empty-state stub. **11 Sept 2026: imported a real 27-week Barcelona Marathon plan** (`scripts/import-barcelona-plan.ts`, `npm run plan:import`) — 99 dated sessions, one row per prescribed day (rest days get no row), Sept 10 2026 → race day Sun 14 Mar 2027. Inserted directly as `status: "planned"`, no `plan_revisions` row — this is baseline data the user authored/chose, not an engine adaptation, so it doesn't go through the propose/approve machinery. Deleted the old `weekly-target` test placeholder from Phase 5 as part of the same script. Revision history / diff badges still deferred — nothing has revised any of these yet.
- [ ] **Follow-up, not yet done:** the weekly-draft cron (`server/utils/weeklyDraft.ts`) still drafts its own generic "weekly-target" volume row, disconnected from this real dated plan — it doesn't know the plan exists, and would currently write a second, competing row alongside real ones. Needs reconciling before the cron runs again for real (next Sunday) or it'll clutter the Plan screen with a redundant entry.
...
- [x] `vercel.json` added with the sync-garmin cron schedule. Daily, not the 2–4h cadence originally sketched — **Vercel's Hobby plan only allows daily-granularity cron schedules**; more frequent syncing needs Pro, or manual triggering in the meantime.
- [ ] **Disabled, 11 Sept 2026:** the weekly-draft cron's schedule was removed from `vercel.json` (code/route left intact, just not auto-triggered) once a real 99-session dated plan existed to import (see Phase 4) — writing a disconnected generic "weekly-target" row no longer makes sense next to real dated sessions. Decision: rebuild this properly later so a progress/regress verdict scales the *actual* upcoming imported sessions, rather than quick-patch the placeholder logic. Until then, the engine's verdict is visible on Today, but nothing auto-drafts changes to specific days.
